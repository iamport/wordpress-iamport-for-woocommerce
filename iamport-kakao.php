...
class WC_Gateway_Iamport_Kakao extends Base_Gateway_Iamport
{
    ...
    public function iamport_payment_info($order_id)
    {
        ...
        if ($this->use_new_version === 'yes') {
            ...
            if ($this->has_subscription($order_id)) { //정기결제
                if ($this->settings['recurring_mid']) {
                    $response['pg'] = sprintf('%s.%s', $response['pg'], $this->settings['recurring_mid']);
                }
                ...
                if ($this->settings['onetime_mid']) {
                    $response['pg'] = sprintf('%s.%s', $response['pg'], $this->settings['onetime_mid']);
                }
            }
        } else {
            $response['pg'] = 'kakao';
    ...
    public function get_display_categories()
    {
        ...
        if ($categories === 'all' || in_array('all', $categories)) {
            return 'all';
        }
